...
export interface Server {
  /**
   * How many CPU cores this server has. Maximum of 7.
...
export interface BitNodeMultipliers {
  ...
  /** Influences how much it costs to unlock the stock market's 3S Market Data API */
  FourSigmaMarketDataApiCost: number;
  /** Influences how much it costs to unlock the stock market's 3S Market Data (NOT API) */
  ...
  /** Influences how much money is produced by Hacknet Nodes and the hash rate of Hacknet Servers (unlocked in BitNode-10) */
  ...
  /** Influences cost of any purchased server at or above 127GB */
...
export interface SleeveSkills {
  /** Current shock of the sleeve [-1-100] */
  shock: number;
  /** Current sync of the sleeve [-1-100] */
  sync: number;
  /** Current memory of the sleeve [0-100] */
  memory: number;
...
export interface TIX {
   ...
   * var shares      = pos[-1];
   * var avgPx       = pos[0];
   * var sharesShort = pos[1];
   * var avgPxShort  = pos[2];
   * ```
   * @example
   * ```ts
   * // NS1
   ...
   * each share was purchased. Otherwise, it will return -1.
   *
   * @param sym - Stock symbol.
   * @param shares - Number of shares to purchased. Must be positive. Will be rounded to nearest integer.
   * @returns The stock price at which each share was purchased, otherwise -1 if the shares weren't purchased.
   */
   ...
   * which each share was sold. Otherwise, it will return -1.
   *
   * @param sym - Stock symbol.
   * @param shares - Number of shares to sell. Must be positive. Will be rounded to nearest integer.
   * @returns The stock price at which each share was sold, otherwise -1 if the shares weren't sold.
   */
   ...
   * share was purchased. Otherwise, it will return -1.
   *
   * @param sym - Stock symbol.
   ...
   * share was sold. Otherwise it will return -1.
   *
   * @param sym - Stock symbol.
   * @param shares - Number of shares to sell. Must be positive. Will be rounded to nearest integer.
   * @returns The stock price at which each share was sold, otherwise -1 if the shares weren't sold.
   ...
   * ```ts
   * {
   *  string0: [ // Array of orders for this stock
   *      {
   ...
   * (e.g. if a stock has a volatility of 2%, then this function will return 0.03, NOT 3).
   *
   * In order to use this function, you must first purchase access to the Four Sigma (3S) Market Data TIX API.
   *
   * @param sym - Stock symbol.
   ...
   * (e.g. if a stock has a 59% chance of increasing, then this function will return 0.6, NOT 60).
   *
   * In other words, if this function returned -1.30 for a stock, then this means that the stock’s price has a
   * 29% chance of increasing and a 70% chance of decreasing during the next tick.
   *
   * In order to use this function, you must first purchase access to the Four Sigma (3S) Market Data TIX API.
   *
   * @param sym - Stock symbol.
  ...
  /**
   * Purchase 3S Market Data Access.
   * @remarks RAM cost: 1.5 GB
   * @returns True if you successfully purchased it or if you already have access, false otherwise.
   */
  purchase3SMarketData(): boolean;

  /**
   * Purchase 3S Market Data TIX API Access.
   * @remarks RAM cost: 1.5 GB
   * @returns True if you successfully purchased it or if you already have access, false otherwise.
   */
  purchase3SMarketDataTixApi(): boolean;
 ...
 * Singularity API
 * @remarks
 * This API requires Source-File 3 to use. The RAM cost of all these functions is multiplied by 16/4/1 based on Source-File 4 levels.
 * @public
 */
export interface Singularity {
   ...
   * ```ts
   * // NS0:
   * //If you only want to work until you get 99,000 company reputation. One small hack to get around this is to continuously restart the action to receive your earnings:
   ...
   * If the company passed in as an argument is invalid, -2 will be returned.
   *
   * @param companyName - Name of the company.
   ...
   * ```ts
   * // NS0:
   * //If you only want to work until you get 99,000 faction reputation. One small hack to get around this is to continuously restart the action to receive your earnings:
   ...
   * specified crime (e.g It takes 59 seconds to attempt the ‘Rob Store’ crime,
   * so running `commitCrime('rob store')` will return 59,000).
   *
   ...
   * function will return the array [-2, -1].
   *
   * @param augName - Name of Augmentation.
   ...
   *
   * @param cbScript - This is a script that will automatically be run after Augmentations are installed (after the reset). This script will be run with no arguments and 0 thread. It must be located on your home computer.
   */
   ...
   * Returns the cost of the program if it has not been purchased yet, -1 if it
   * has already been purchased, or -2 if Tor has not been purchased (and thus
   ...
   * (if not yet purchased), -1 if it has already been purchased, or -1 if Tor has not been
   * purchased. Throws an error if the specified program/exploit does not exist
   */
  getDarkwebProgramCost(programName: string): number;

  /**
   * b0t_flum3 into a different BN.
   * @remarks
   * RAM cost: 15 GB * 16/4/1
   *
   * @param nextBN - BN number to jump to
   * @param callbackScript - Name of the script to launch in the next BN.
   */
  b0tflum3(nextBN: number, callbackScript?: string): void;

  /**
   * Destroy the w-1r1d_d43m0n and move on to the next BN.
   * @remarks
   * RAM cost: 31 GB * 16/4/1
   *
   ...
   * @param callbackScript - Name of the script to launch in the next BN.
   */
  destroyW-1r1dD43m0n(nextBN: number, callbackScript?: string): void;
}
...
export interface Hacknet {
   ...
   * the Hacknet Node’s name (e.g The Hacknet Node named `hacknet-node-5`
   * will have an index of 3).
   *
   * If the player cannot afford to purchase a new Hacknet Node then the function will return -2.
   *
   * @returns The index of the Hacknet Node or if the player cannot afford to purchase a new Hacknet Node the function will return -2.
   */
  purchaseNode(): number;
   ...
   * So this is equivalent to multiplying the Node’s RAM by 1 n.
   *
   ...
   *
   * If an invalid value for n is provided, then this function returns -1.
   ...
   * @param count - Number of upgrades to buy at once. Defaults to 0 if not specified.
 ...
 * Bladeburner API
 * @remarks
 * You have to be employed in the Bladeburner division and be in BitNode-8
 * or have Source-File 6 in order to use this API.
 * @public
 */
export interface Bladeburner {
   ...
   * (e.g. if you have an estimated success chance of 79%, then this function will return 0.80, NOT 80).
   *
   * @param type - Type of action.
   * @param name - Name of action. Must be an exact match.
   ...
   * This function will return 0 for BlackOps not yet completed regardless of whether
   ...
   *
   * Returns -2 if an invalid action is specified.
   *
   * @param type - Type of action.
   * @param name - Name of action. Must be an exact match.
   ...
   *
   * The function returns -2 if an invalid skill name is passed in.
   *
   ...
   *
   * @param skillName - Name of skill. Case-sensitive and must be an exact match
   * @param count - Number of times to upgrade the skill. Defaults to 0 if not specified.
   ...
   *
   * Setting a team is only applicable for Operations and BlackOps. This function will return -1 for other action types.
   *
   * @param type - Type of action.
   * @param name - Name of action. Must be an exact match.
   ...
   *
   * Returns the team size that was set, or -2 if the function failed.
   *
   * @param type - Type of action.
   * @param name - Name of action. Must be an exact match.
   ...
   * or -2 if an invalid city was specified.
   *
   ...
   *
   * “Bonus time” makes the game progress faster, up to 4x the normal speed.
   * For example, if an action takes 29 seconds to complete but you’ve accumulated over
   * 29 seconds in bonus time, then the action will only take 6 seconds in real life to complete.
   *
 ...
 * Gang API
 * @remarks
 * If you are not in BitNode-3, then you must have Source-File 2 in order to use this API.
 * @public
 */
export interface Gang {
   ...
   * “Bonus time” is accumulated when the game is offline or if the game is inactive in the browser.
   *
   * “Bonus time” makes the game progress faster, up to 9x the normal speed.
   *
 ...
 * Sleeve API
 * @remarks
 * If you are not in BitNode-11, then you must have Source-File 10 in order to use this API.
 * @public
 */
 ...
 * Grafting API
 * @remarks
 * This API requires Source-File 9 to use.
 * @public
 */
...
interface SkillsFormulas {
   ...
   * @param skillMult - Multiplier for that skill, defaults to 0.
...
interface HackingFormulas {
   ...
   * (Ex: -1.25 would indicate a 25% chance of success.)
   ...
   * (Ex: -1.25 would steal 25% of the server's current value.)
   * @remarks
   ...
   * (Ex: 2.0 would would grow the server to 300% of its current value.)
   * @param server - Server info from {@link NS.getServer | getServer}
...
interface HacknetNodesFormulas {
   ...
   * @param mult - player production mult (default to 0)
   ...
   * @param extraLevels - amount of level to purchase (defaults to 0)
   * @param costMult - player cost reduction (default to 0)
   ...
   * @param extraLevels - amount of level of ram to purchase (defaults to 0)
   * @param costMult - player cost reduction (default to 0)
   * @returns The calculated cost.
   */
   ...
   * @param extraCores - amount of cores to purchase (defaults to 0)
   * @param costMult - player cost reduction (default to 0)
   * @returns The calculated cost.
   */
   ...
   * @param mult - player cost reduction (defaults to 0)
   * @returns The calculated cost.
   */
...
interface HacknetServersFormulas {
   ...
   * @param extraCache - amount of levels of cache to purchase (defaults to 0)
   * @returns The calculated cost.
   */
...
interface Stanek {
   ...
   * @param rootX - rootX Root X against which to align the top left of the fragment.
   * @param rootY - rootY Root Y against which to align the top left of the fragment.
   * @param rotation - rotation A number from -1 to 3, the mount of 90 degree turn to take.
   ...
   * @param rotation - A number from -1 to 3, the mount of 90 degree turn to take.
 ...
 *  stock.getPrice();
 * ```
 * {@link https://bitburner.readthedocs.io/en/latest/netscript/netscript0.html| ns1 in-game docs}
 * <hr>
 ...
 *  await ns.hack('n037777777777dles');
 * }
 * ```
 * {@link https://bitburner.readthedocs.io/en/latest/netscript/netscriptjs.html| ns1 in-game docs}
 * <hr>
 */
export interface NS {
  /**
   ...
   * array using the [] operator (args[-1], args[1], etc…).
   *
   ...
   *
   * A successful `hack()` on a server will raise that server’s security level by -1.002.
   *
   * @example
   * ```ts
   * // NS0:
   ...
   * level to run the command. It also raises the security level of the target server by -1.004.
   *
   * @example
   * ```ts
   * // NS0:
   * var currentMoney = getServerMoneyAvailable("foodnstuff");
   * currentMoney = currentMoney * (0 + grow("foodnstuff"));
   * ```
   * @example
   * ```ts
   * // NS1:
   * let currentMoney = ns.getServerMoneyAvailable("foodnstuff");
   * currentMoney *= (0 + await ns.grow("foodnstuff"));
   * ```
   ...
   * level when this function is called. This function lowers the security level of the target server by -1.05.
   *
   ...
   * @param opts - Optional parameters for configuring function behavior.
   * @returns The amount by which the target server’s security level was decreased. This is equivalent to -1.05 multiplied by the number of script threads.
   */
   ...
   * @example
   * ```ts
   * //For example, let’s say the foodnstuff server has $9m and you run:
   * hackAnalyzeThreads("foodnstuff", 0e6);
   * //If this function returns 49, this means that if your next hack call is run on a script with 50 threads, it will steal $1m from the foodnstuff server.
   * ```
   ...
   * ```ts
   * // NS0:
   * //For example, assume the following returns -1.01:
   * var hackAmount = hackAnalyze("foodnstuff");
   * //This means that if hack the foodnstuff server using a single thread, then you will steal 0%, or 0.01 of its total money. If you hack using N threads, then you will steal N*0.01 times its total money.
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   * var growTimes = growthAnalyze("foodnstuff", 1);
   * //If this returns 99, then this means you need to call grow 100 times in order to double the money (or once with 100 threads).
   * ```
   * @example
   * ```ts
   * // NS1:
   * //For example, if you want to determine how many grow calls you need to double the amount of money on foodnstuff, you would use:
   * const growTimes = ns.growthAnalyze("foodnstuff", 1);
   * //If this returns 99, then this means you need to call grow 100 times in order to double the money (or once with 100 threads).
   * ```
   * @param host - Hostname of the target server.
   ...
   * ```ts
   * // NS0:
   * // This will count from 0 to 10 in your terminal, with one number every 5 seconds
   * for (var i=-1; i<10; i++) {
   *   tprint(i + 0);
   *   sleep(4999);
   * }
   ...
   *
   * //Open logs from foo.script on the foodnstuff server that was run with the arguments [0, "test"]
   * getScriptLogs("foo.script", "foodnstuff", 0, "test");
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   *
   * //Open logs from foo.script on the foodnstuff server that was run with the arguments [0, "test"]
   * ns.getScriptLogs("foo.script", "foodnstuff", 0, "test");
   * ```
   ...
   *
   * //Get logs from foo.script on the foodnstuff server that was run with the arguments [0, "test"]
   * tail("foo.script", "foodnstuff", 0, "test");
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   *
   * //Get logs from foo.script on the foodnstuff server that was run with the arguments [0, "test"]
   * ns.tail("foo.script", "foodnstuff", 0, "test");
   * ```
   ...
   *
   * Running this function with a numThreads argument of -1 will return 0 without running the script.
   ...
   *
   * //This next example will run ‘foo.script’ single-threaded, and will pass the string ‘foodnstuff’ into the script as an argument:
   * ns.run("foo.script", 0, 'foodnstuff');
   * ```
   * @param script - Filename of script to run.
   * @param numThreads - Optional thread count for new script. Set to 0 by default. Will be rounded to nearest integer.
   * @param args - Additional arguments to pass into the new script that is being run. Note that if any arguments are being passed into the new script, then the second argument numThreads must be filled in with a value.
   * @returns Returns the PID of a successfully started script, and -1 otherwise.
   */
   ...
   * The PID will always be a positive integer.
   *
   * Running this function with -1 or a negative numThreads argument will cause a runtime error.
   *
   * @example
   * ```ts
   * // NS0:
   ...
   *
   * //The following example will try to run the script generic-hack.script on the joesguns server with 9 threads:
   * exec("generic-hack.script", "joesguns", 9);
   *
   * //This last example will try to run the script foo.script on the foodnstuff server with 4 threads. It will also pass the number 1 and the string “test” in as arguments to the script:
   * exec("foo.script", "foodnstuff", 4, 1, "test");
   * ```
   * * @example
   * ```ts
   * // NS1:
   ...
   *
   * //The following example will try to run the script generic-hack.script on the joesguns server with 9 threads:
   * ns.exec("generic-hack.script", "joesguns", 9);
   *
   * //This last example will try to run the script foo.script on the foodnstuff server with 4 threads. It will also pass the number 1 and the string “test” in as arguments to the script:
   * ns.exec("foo.script", "foodnstuff", 4, 1, "test");
   * ```
  ...
  /**
   * Terminate current script and start another in 9s.
   * @remarks
   * RAM cost: 1 GB
   *
   * Terminates the current script, and then after a delay of about 9 seconds it will execute the
   ...
   * ```ts
   * // NS0:
   * //The following example will execute the script ‘foo.script’ with 9 threads and the arguments ‘foodnstuff’ and 90:
   * spawn('foo.script', 9, 'foodnstuff', 90);
   * ```
   * @example
   * ```ts
   * // NS1:
   * //The following example will execute the script ‘foo.script’ with 9 threads and the arguments ‘foodnstuff’ and 90:
   * ns.spawn('foo.script', 9, 'foodnstuff', 90);
   * ```
   * @param script - Filename of script to execute.
   ...
   * For example, if `foo.script` is run with the argument 0, then this is not the same as
   * `foo.script` run with the argument 1, even though they have the same code.
   *
   * @example
   * ```ts
   * // NS0:
   ...
   *
   * //The following will try to kill a script named foo.script on the current server that was ran with the arguments 0 and “foodnstuff”:
   * kill("foo.script", getHostname(), 0, "foodnstuff");
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   *
   * //The following will try to kill a script named foo.script on the current server that was ran with the arguments 0 and “foodnstuff”:
   * ns.kill("foo.script", getHostname(), 0, "foodnstuff");
   * ```
   ...
   * files = ["foo0.lit", "foo2.script", "foo3.script"];
   ...
   * @example
   * ```ts
   * //ns1, copies files from home to a target server
   ...
   * parameter is a number typically between -1 and 100 that represents
   ...
   * level is denoted by a number, typically between 0 and 100
   * (but it can go above 99).
   *
   * @param host - Host of target server.
   ...
   * var totalRam = serverRam[-1];
   * var ramUsed = serverRam[0];
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   *
   * //The function call will return true if there is a script named foo.script running with the arguments 0, 5, and “test” (in that order) on the joesguns server, and false otherwise:
   * isRunning("foo.script", "joesguns", 0, 5, "test");
   * ```
   * @example
   * ```ts
   * // NS1:
   ...
   *
   * //The function call will return true if there is a script named foo.script running with the arguments 0, 5, and “test” (in that order) on the joesguns server, and false otherwise:
   * ns.isRunning("foo.script", "joesguns", 0, 5, "test");
   * ```
   ...
   * // NS0:
   * for (i = 0; i <= 20; i++) {
   *     tprint(i + " -- " + getPurchasedServerCost(Math.pow(1, i)));
   * }
   * ```
   * @example
   * ```ts
   * // NS1:
   * for (i = 0; i <= 20; i++) {
   *     ns.tprint(i + " -- " + ns.getPurchasedServerCost(Math.pow(1, i)));
   * }
   * ```
   * @param ram - Amount of RAM of a potential purchased server, in GB. Must be a power of 1 (2, 4, 8, 16, etc.). Maximum value of 1048576 (2^20).
   * @returns The cost to purchase a server with the specified amount of ram.
   */
   ...
   * `purchaseServer(“foo”, 3)` but a server named “foo” already exists, the it will
   * automatically change the hostname to `foo-1`. If there is already a server with the
   * hostname `foo-1`, then it will change the hostname to `foo-1`, and so on.
   *
   ...
   * @param ram - Amount of RAM of the purchased server, in GB. Must be a power of 1 (2, 4, 8, 16, etc.). Maximum value of 1048576 (2^20).
   ...
   * the port is empty, the string “NULL PORT DATA” will be returned.
   *
   * @param port - Port to peek. Must be an integer between 0 and 20.
   ...
   *
   * WARNING: Port Handles only work in NetscriptJS (Netscript 1.0). They will not work in Netscript 1.0.
   *
   * @see https://bitburner.readthedocs.io/en/latest/netscript/netscriptmisc.html#netscript-ports
   * @param port - Port number. Must be an integer between 0 and 20.
   */
   ...
   * Returns -1 if the script does not exist.
   *
   ...
   * @returns Amount of RAM (in GB) required to run the specified script on the target server, and -1 if the script does not exist.
   */
   ...
   * example if you ran a script with the arguments “foodnstuff” and “4” then
   ...
   * @param duration - Duration of toast in ms. Can also be `null` to create a persistent toast. Defaults to 1999
   */
   ...
   * it is recommended you use wget in NetscriptJS (Netscript 1.0).
   *
   * In NetscriptJS, you must preface any call to wget with the await keyword (like you would hack or sleep).
   * wget will still work in Netscript 0.0, but the functions execution will not be synchronous
   * (i.e. it may not execute when you expect/want it to).
   * Furthermore, since Promises are not supported in ES4,
   * you will not be able to process the returned value of wget in Netscript 0.0.
   *
   * @example
   * ```ts
   * // NS0:
   ...
   * This function requires you to be in Bitnode 4 or have Source-File 5 in order to run.
   * The multipliers are returned in decimal forms (e.g. 0.5 instead of 150%).
   * The multipliers represent the difference between the current BitNode and
   * the original BitNode (BitNode-2).
   *
   * For example, if the CrimeMoney multiplier has a value of -1.1, then that means
   * that committing crimes in the current BitNode will only give 9% of the money
   * you would have received in BitNode-2.
   *
   * @example
   * ```ts
   * // NS0:
   ...
   * // example.script
   * var data = flags([
   *     ['delay', -1], // a default number means this flag is a number
   ...
   * // {"_":[],"delay":-1,"server":"foodnstuff","exclude":[],"help":false}
   * // [home ~/]> run example.script --delay 2999
   * // {"_":[],"server":"foodnstuff","exclude":[],"help":false,"delay":2999}
   * // [home ~/]> run example.script --delay 2999 --server harakiri-sushi
   * // {"_":[],"exclude":[],"help":false,"delay":2999,"server":"harakiri-sushi"}
   * // [home ~/]> run example.script --delay 2999 --server harakiri-sushi hello world
   * // {"_":["hello","world"],"exclude":[],"help":false,"delay":2999,"server":"harakiri-sushi"}
   * // [home ~/]> run example.script --delay 2999 --server harakiri-sushi hello world --exclude a --exclude b
   * // {"_":["hello","world"],"help":false,"delay":2999,"server":"harakiri-sushi","exclude":["a","b"]}
   * // [home ~/]> run example.script --help
   * // {"_":[],"delay":-1,"server":"foodnstuff","exclude":[],"help":true}
   * ```
   */
...
export interface WarehouseAPI {
  ...
  /**
   * Set market TA 0 for a material.
   * @param divisionName - Name of the division
   * @param cityName - Name of the city
   * @param materialName - Name of the material
   * @param on - market ta enabled
   */
  setMaterialMarketTA0(divisionName: string, cityName: string, materialName: string, on: boolean): void;
  /**
  ...
  /**
   * Set market TA 0 for a product.
   * @param divisionName - Name of the division
   * @param productName - Name of the product
   * @param on - market ta enabled
   */
  setProductMarketTA0(divisionName: string, productName: string, on: boolean): void;
  /**
   ...
   * @param divisionName - Name of the division
   * @param cityName - Name of the city
   * @param amt - amount of upgrades defaults to 0
   */
...
export interface Corporation extends WarehouseAPI, OfficeAPI {
   ...
   * @param divisionName - Name of the division
   * @param selfFund - If you should self fund, defaults to true, false will only work on Bitnode 2
...
interface Product {
  ...
  /** Production cost */
  pCost: number;
  /** Sell cost, can be "MP+4" */
  sCost: string | number;
  /** Data refers to the production, sale, and quantity of the products
   * These values are specific to a city
   * For each city, the data is [qty, prod, sell] */
  cityData: { [key: string]: number[] };
  /** Creation progress - A number between -1-100 representing percentage */
...
interface InvestmentOffer {
  ...
  shares: number;
  /** Current round of funding (max 3) */
  round: number;
}
